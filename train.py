...
if __name__ == "__main__":
    ...
    if args.task == "domain_adaptation" and not uda_splits:
        raise ValueError("Not enough unlabeled samples for domain adaptation.")

    train_loaders = [
        InfiniteDataLoader(env, env_weights, hparams['batch_size'])
    ...
    eval_loaders = [
        FastDataLoader(env, batch_size=64)
    ...
    for step in range(n_steps):
        ...
        uda_device = [x.to(device) for x, _ in next(uda_minibatches_iterator)] if args.task == "domain_adaptation" else None
