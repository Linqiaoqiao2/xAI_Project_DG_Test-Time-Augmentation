# -*- coding: utf-8 -*-
"""
Evaluate ResNet18 (MVDG-train-only models) on PACS with:
...
SEEDS = [42, 123, 2024]
MODEL_DIR = "/home/proj25gr5/xAI_neu/models_mvdg_train_only"
BASE_PACS_PATH = "/home/proj25gr5/xAI/kfold"
SAVE_ROOT = f"/home/proj25gr5/xAI_neu/TTA_results_mvdg_eval_{datetime.now().strftime('%Y%m%d_%H%M')}"
...
for domain in PACS_DOMAINS:
    ...
    for seed in SEEDS:
        model_path = os.path.join(MODEL_DIR, f"resnet18_seed{seed}_target{domain}.pth")
...
with open(os.path.join(SAVE_ROOT, "tta_vs_noaug_results.json"), "w") as f:
    json.dump({"per_seed": results, "avg": avg_results}, f, indent=4)

print(f"\n✅ Results saved to {SAVE_ROOT}/tta_vs_noaug_results.json")
...
ax.set_title('TTA vs No Augmentation (MVDG-trained Models)')
ax.set_xticks(x + bar_width / 2)
ax.set_xticklabels(PACS_DOMAINS)
ax.set_ylim(0, 1.0)
ax.legend()
plt.tight_layout()
plot_path = os.path.join(SAVE_ROOT, "tta_vs_noaug_plot.png")
